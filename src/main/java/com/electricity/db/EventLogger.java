...
public class EventLogger {
    ...
    public static void logEvent(String nodeId, String eventType, String metadata) {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            String sql = "INSERT INTO events (event_id, node_id, event_type, timestamp, metadata) VALUES (?, ?, ?, ?, ?)";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                ps.executeUpdate();
                System.out.println("[Event-Logger] SUCCESS: Logged " + eventType + " for " + nodeId);
            }
        } catch (Exception e) {
